...
엔티티는 Bevy 월드 안의 ID입니다. 그 ID에 붙은 컴포넌트가 데이터를 담고, 시스템이 동작을 제공합니다.
...
이 반복문은 쿼리 결과를 순회하는 일반 Rust 반복문입니다.
...
이 `.0`은 Rust tuple field 문법입니다.

## Bevy로 보면

ECS는 타입 있는 데이터 열을 저장합니다. 시스템은 필요한 데이터 열을 선언해서 가져가고, Bevy는 그 선언을 보고 함께 실행해도 되는 시스템을 판단합니다.
