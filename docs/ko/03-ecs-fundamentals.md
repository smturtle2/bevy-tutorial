...
컴포넌트가 엔티티에 위치, 체력, 속도, 렌더링, 행동을 부여합니다. 엔티티 자체는 그 컴포넌트들이 붙는 ID입니다.
...
컴포넌트 타입이 엔티티의 형태를 정합니다. 쿼리는 spawn 순서가 아니라 컴포넌트 형태로 엔티티를 찾습니다.
...
두 쿼리 모두 `Transform`에 접근합니다. 플레이어 집합과 카메라 집합이 분리되어 있음을 증명하려면 필터를 추가합니다.
...
`Local<T>`는 시스템 로컬 상태를 저장합니다. `Local<T>`를 요청하는 각 시스템은 자기만의 지속 값을 받습니다.
...
한 시스템에만 속하고 그 시스템 내부에 머무는 상태라면 `Local`을 씁니다.
...
Transform을 수정합니다.
Velocity를 읽습니다.
엔티티 생성과 제거는 다른 시스템에 맡깁니다.
키보드 입력은 다른 시스템에 맡깁니다.
체력과 점수는 다른 시스템에 맡깁니다.
```
