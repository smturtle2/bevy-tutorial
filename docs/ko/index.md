...
> [!IMPORTANT]
> 이 튜토리얼은 첫 Bevy 예제부터 Rust를 함께 가르칩니다. Rust 문법은 예제가 필요로 하는 순서에 맞춰 등장하고, 각 장은 실행 가능한 예제 하나와 연결됩니다.
...
> [!TIP]
> 한 장을 읽고, 해당 예제를 실행하고, 코드를 조금 바꿔본 뒤 다음 장으로 넘어가세요. 최종 코드는 분석하고 고쳐 보는 대상으로 다룹니다. Bevy는 "이 시스템이 어떤 데이터를 읽고, 어떤 데이터를 쓰는가?"를 반복해서 물을 때 가장 배우기 쉽습니다.
