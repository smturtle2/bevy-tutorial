...
이 튜토리얼은 Rust와 Bevy를 하나의 흐름으로 배웁니다. 빈 Bevy 창에서 시작해 부드러운 카메라 추적, 적 웨이브, 공격 hitbox, sprite asset, screen-space UI, 애니메이션 상태, 직접 만든 맵 지오메트리, 게임 상태, 저장/불러오기를 포함한 작은 탑다운 RPG까지 만듭니다.
...
- `Commands`, `Res`, `Query`, `With`, `Without`를 사용하는 Bevy 시스템을 읽고 작성한다.
- 작은 컴포넌트 타입을 설계하고, 엔티티는 번들로 생성한다.
- 게임 기능을 플러그인과 모듈로 나눈다.
- `.chain()`과 `SystemSet`으로 시스템 실행 순서를 제어한다.
- Bevy 코드에 나오는 Rust 개념을 설명한다: `struct`, tuple struct, `impl`, trait, derive, ownership, reference, module, visibility.
...
10. [공격 hitbox](10-attack-hitbox.md)
11. [sprite asset](11-sprite-assets.md)
